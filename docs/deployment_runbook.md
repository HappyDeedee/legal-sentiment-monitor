...
- 邮箱服务商要求授权码，不是登录密码。
- 默认收件人为空。
- `MONITOR_ALLOW_REAL_EMAIL_SEND` 未显式开启，系统按安全策略跳过真实外发。

处理方式：先确认 SMTP 配置、发件人、授权码和默认收件人。默认本地、测试和诊断路径不会发送真实邮件；只有在试运行/生产验证窗口内显式设置
`MONITOR_ALLOW_REAL_EMAIL_SEND=true`，并确认收件人、发送时间和回滚记录后，才允许在邮件配置页发送测试邮件并确认收件箱收到邮件。验证结束后应恢复默认非发送状态。

### 没有明显运行任务却收到报告邮件

先按证据排查，不要直接删除数据库记录、报告文件或邮件文件。

只读检查顺序：

1. 保存收到的 `.eml` 文件或邮件原文，记录主题、发送时间、附件名和收件人。
2. 在报告发送记录中查找对应的发送时间、`job_id`、`report_id`、发送类型和状态。
3. 检查报告目录中是否还有同名 HTML、Excel、Markdown 或附件文件。
