...
- 报告中心：预览 HTML，下载 HTML、Excel 和 Markdown，并按律所、平台、风险、日期筛选；风险筛选支持高风险、疑似负面、待人工复核、无风险。下方会展示线索明细，方便运营逐条查看标题、链接、封面、AI 理由、证据和待人工复核状态；也可以生成本地自测报告，检查报告链路是否正常。
...
服务器运行前，需要分别准备三个平台的浏览器 profile。默认 profile 路径：
...
- “三平台真实采集”必须看到抖音、快手、小红书都至少有一次成功采集结果；只有某一个平台成功时，验收状态仍会提示缺少的平台。
...
1. 在平台状态里确认抖音、快手、小红书 Profile 都已发现。
...
6. 回到任务管理页刷新验收状态，确认“三平台真实采集”不再提示缺口。
